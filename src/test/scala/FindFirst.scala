...
class FindFirst extends FunSuite {
  ...
  test("Email") {
    ...
    assert(findFirst[Email]("bar abc@def.com hi hello bar@foo.com") == Some("abc@def.com"))
    assert(findFirst[Email]("sdsdsd@sdf.com") == Some("sdsdsd@sdf.com"))
    assert(findFirst[Email]("ddddd") == None)
  }

  test("italian fiscal code") {
    import com.github.gekomad.regexcollection.ItalianFiscalCode
    import com.github.gekomad.regexcollection.Validate.findFirst
    assert(findFirst[ItalianFiscalCode]("bar bdAPPP14A01A001R sdfdfgdfgw BDAPPP14A01A001R d ") == Some("bdAPPP14A01A001R"))
  }

  test("NotASCII") {
    import com.github.gekomad.regexcollection.NotASCII
    import com.github.gekomad.regexcollection.Validate.findFirst
    assert(findFirst[NotASCII]("""    テ ス  ト      a ああ   bw""") == Some("テ ス  ト      "))
  }

  test("Percentage") {
    import com.github.gekomad.regexcollection.Percentage
    import com.github.gekomad.regexcollection.Validate.findFirst
    assert(findFirst[Percentage]("""dd 10%  dg 55% """) == Some("10%"))
  }

  test("MACAddress") {
    import com.github.gekomad.regexcollection.MACAddress
    import com.github.gekomad.regexcollection.Validate.findFirst
    assert(findFirst[MACAddress]("""
        ...
        |        TX errors 0  dropped 0 overruns 0  carrier 0  collisions 0
        |
      """.stripMargin) == Some("ec:f4:bb:1f:d1:fc"))
  }
